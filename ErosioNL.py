def ErosioNL(X, Y, h, final_time, delta_t_max, delta_t0, cr_angle, enne, k,
    ...
    # multiply k.
    #
    # - X and Y are the (ny,nx) array of the coordinates as generated by the
    #   matlab meshgrid command;
    # - h is the (ny,nx) initial elevation array;
    ...
    # - k is the diffusion goefficient, a scalar or a (ny,nx) array. The
    ...
    h_old = np.zeros((ny, nx))
    h_new = np.zeros((ny, nx))
    h_temp = np.zeros((ny, nx))
    h_temp_half = np.zeros((ny, nx))
    h_temp_full = np.zeros((ny, nx))
    ...
    while (simtime < final_time):
        ...
        h[0:ny, 0:nx] = h_temp_half[0:ny, 0:nx]
    ...
    h_new[0:ny, 0:nx] = h[0:ny, 0:nx]

    total_area = (X[0, nx - 1] - X[0, 0]) * (Y[ny - 1, 0] - Y[0, 0])
